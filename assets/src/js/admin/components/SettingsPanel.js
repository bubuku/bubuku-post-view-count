	...
	const handleSave = () => {
		...
		bbkFetch( '/settings', {
			...
			.then( ( saved ) => {
				...
				setStatus( {
					type: 'green-500',
					message: __( 'Ajustes guardados.', 'bubuku-post-view-count' ),
				} );
			} )
			.catch( () => {
				setSaving( false );
				setStatus( {
					type: 'error',
					message: __( 'No se han podido guardar los ajustes.', 'bubuku-post-view-count' ),
				} );
			} );
	};
	...
	const handleDeleteData = () => {
		...
		bbkFetch( '/settings/data', { method: 'DELETE' } )
			.then( () => {
				...
				setStatus( {
					type: 'green-500',
					message: __( 'Se han eliminado todas las vistas registradas.', 'bubuku-post-view-count' ),
				} );
			} )
			.catch( () => {
				setDeleting( false );
				setStatus( {
					type: 'error',
					message: __( 'No se han podido eliminar los datos.', 'bubuku-post-view-count' ),
				} );
			} );
	};
	...
	return (
		<div className="bbk-settings-panel">
			<DashboardCard
				...
				claim={ __( 'Tipos de contenido y visitantes que generan una vista.', 'bubuku-post-view-count' ) }
			>
				<div className="bbk-field-group">
					...
					{ Object.entries( postTypes ).map( ( [ slug, label ] ) => (
						<label key={ slug } className="bbk-checkbox">
							<input
								type="checkbox"
								checked={ ( settings.post_types || [] ).includes( slug ) }
								onChange={ () => toggleInList( 'post_types', slug ) }
							/>
							{ label }
						</label>
					) ) }
				...
				<div className="bbk-field-group">
					...
					{ Object.entries( roles ).map( ( [ slug, label ] ) => (
						<label key={ slug } className="bbk-checkbox">
							<input
								type="checkbox"
								checked={ ( settings.excluded_roles || [] ).includes( slug ) }
								onChange={ () => toggleInList( 'excluded_roles', slug ) }
							/>
							{ label }
						</label>
					) ) }
				...
				<div className="bbk-field-group">
					<label className="bbk-checkbox">
						<input
							type="checkbox"
							checked={ !! settings.exclude_bots }
							onChange={ ( e ) => update( 'exclude_bots', e.target.checked ) }
						/>
						{ __( 'No contar visitas de user-agents de bots conocidos.', 'bubuku-post-view-count' ) }
					</label>
					<p className="bbk-field-group__help">
						{ __( 'Incluye, entre otros:', 'bubuku-post-view-count' ) }{ ' ' }
			...
			<DashboardCard
				...
				claim={ __( 'Rastreo de bots de IA y señales de privacidad del visitante.', 'bubuku-post-view-count' ) }
				animationDelay=".1s"
			>
				<div className="bbk-field-group">
					<label className="bbk-checkbox">
						<input
							...
							onChange={ ( e ) => update( 'ai_crawler_tracking', e.target.checked ) }
						/>
						{ __(
				...
				<div className="bbk-field-group">
					<label className="bbk-checkbox">
						<input
							...
							onChange={ ( e ) => update( 'respect_dnt', e.target.checked ) }
						/>
						{ __(
			...
			<DashboardCard
				icon={ ICON_SETTINGS }
				title={ __( 'Rendimiento y retención', 'bubuku-post-view-count' ) }
				claim={ __( 'Escrituras en base de datos e histórico diario.', 'bubuku-post-view-count' ) }
				animationDelay=".15s"
			>
				<div className="bbk-field-group">
					<label className="bbk-checkbox">
						<input
							...
							onChange={ ( e ) => update( 'write_buffer', e.target.checked ) }
						/>
						{ __(
				...
				<div className="bbk-field-group">
					<label className="bbk-field-group__label" htmlFor="bbk-retention-days">
						{ __( 'Retención del agregado diario', 'bubuku-post-view-count' ) }
					</label>
			...
			<BkSaveBar
				...
				labelReset={ __( 'Descartar cambios', 'bubuku-post-view-count' ) }
			/>

			<DashboardCard
				icon={ ICON_DANGER }
				iconVariant="icon-warning"
				title={ __( 'Eliminar todos los datos', 'bubuku-post-view-count' ) }
				claim={ __(
				...
				<div className="bbk-field-group">
					<label className="bbk-checkbox">
						<input
							...
							onChange={ ( e ) => update( 'delete_data_on_uninstall', e.target.checked ) }
						/>
						{ __( 'Eliminar todas las tablas, meta y opciones del plugin al desinstalarlo.', 'bubuku-post-view-count' ) }
					</label>
				</div>
				...
				<div className="bk-card-footer">
					...
					>
						{ deleting
							...
							: __( 'Eliminar todos los datos ahora', 'bubuku-post-view-count' ) }
					</button>
				</div>
			</DashboardCard>
		</div>
