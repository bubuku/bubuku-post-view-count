...
| `Core\Db` | `src/Core/Db.php` | Acceso a datos — upserts atómicos en las tablas propias (incluidas las dimensiones de sesión opcionales de `record_view()`), espejo en post meta, lectura de estadísticas, borrado (meta y tablas) al desinstalar. `record_ai_crawl()` (F6) es un upsert independiente sobre `bbk_post_ai_crawls`, deliberadamente separado de `record_view()` — nunca toca el agregado, el diario, las dims ni el espejo de meta |
...
| `Admin\Settings` | `src/Admin/Settings.php` | Lectura/sanitización de la option `bbk_postview_settings` — CPT habilitados, roles excluidos, filtro de bots, retención, borrado al desinstalar, rastreo de crawlers de IA (F6, `ai_crawler_tracking()`, desactivado por defecto). Única fuente de `enabled_post_types()`, consumida por `Frontend\Assets` y `Api\RestApi` |
...
Options: `bbk_schema_version` (versión de esquema instalada) y `bbk_postview_settings` (array gestionado por `Admin\Settings`/`Admin\SettingsPage` — Fase 2 de `docs/ANALYTICS-PLAN.md`: `post_types`, `excluded_roles`, `exclude_bots`, `retention_days`, `delete_data_on_uninstall`, `ai_crawler_tracking`).
