...
| `Api\TrendsApi` | `src/Api/TrendsApi.php` | Rutas REST `GET /bbk_postview/v1/trends` y `GET /bbk_postview/v1/trends/momentum`, capability `edit_posts` (`docs/ANALYTICS-PLAN.md` §4, F4/§5) — delegan en `Core\Query::trend()` y `Core\Query::momentum()` respectivamente. Separada de `Api\RestApi` a propósito: esa clase es el contador público anónimo con su propio modelo de seguridad; esta es de lectura y con capability, un concern distinto |
...
| `Core\Query` | `src/Core/Query.php` | Consultas de solo lectura sobre las tablas propias — `most_viewed()`, `stale()`, `post_stats()`, `trend()`, `summary()`, `momentum()`. `post_types` siempre intersectado con `Settings::enabled_post_types()`, `limit` con tope duro de 100, resultados con cache corta de objeto (5 min) donde aplica. Sin dependencia de MCP — reutilizable por la página de ajustes, WP-CLI o una futura tool (`docs/ANALYTICS-PLAN.md` §4.1). `momentum()` compara dos periodos consecutivos de igual longitud sobre `bbk_post_views_daily` y ordena por variación **absoluta**, no porcentual (§5) |
...
Las seis tools extienden `BubukuConex\Abstract_Satellite_Tool` (clase del hub), así que solo pueden cargarse con el hub presente — el autoloader propio del plugin (`bbk_autoload`) ya es perezoso por diseño (`spl_autoload_register`), así que basta con instanciarlas solo dentro de `SatelliteConnector::register_tools()` para cumplir esa regla del contrato, sin mecanismo adicional.
...
- `assets/js/admin-stats.js` + `assets/css/admin-stats.css` — gráfica de evolución, comparativa de periodo y listados «en alza»/«en caída» en Ajustes → Post View Count (`docs/ANALYTICS-PLAN.md` F4 UI, §5). `fetch()` contra `GET /bbk_postview/v1/trends` y `GET /bbk_postview/v1/trends/momentum` con el nonce de `wp_rest`; el gráfico se pinta a mano con la Canvas 2D API, sin librería externa. Encolados solo en esa página admin.
