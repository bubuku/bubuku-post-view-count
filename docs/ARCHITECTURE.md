...
La arquitectura actual conserva la analítica, UI React, bloque, WP-CLI y satélite MCP construidos hasta 1.2.2. La revisión de fiabilidad posterior añade el esquema v4 y convierte el camino de escritura en una operación transaccional con deduplicación durable. El buffer experimental de object cache se retiró porque podía perder vistas; el historial de aquella decisión permanece en `docs/ANALYTICS-PLAN.md` y `docs/CHANGELOG.md`.
...
| `Core\Schema` | `src/Core/Schema.php` | Esquema v4 — cinco tablas verificadas tras `dbDelta()`, migración desde `postmeta` por cursor `meta_id` con estado/watchdog, y purgas limitadas con continuación |
...
| `Admin\Settings` | `src/Admin/Settings.php` | Lectura/sanitización de `bbk_postview_settings`: CPT, roles, bots, retención limitada a 3650 días, borrado, crawlers IA y DNT/Sec-GPC. El antiguo buffer opt-in se retiró por no ofrecer persistencia durable |
...
6. `Core\Db::record_unique_view()` crea un HMAC opaco y reclama su fila con clave única. Claim, agregado, diario y dimensiones comparten transacción; un fallo revierte todo y devuelve 503. Los duplicados no incrementan y los excesos por red/post devuelven 429. Tras el commit se actualiza el espejo postmeta.
...
La fuente de verdad son cinco tablas propias, creadas por `Core\Schema` vía `dbDelta()`:
...
| `{prefix}bbk_post_view_dedupe` | `token_hash` | `network_hash`, `post_id`, `created_at`, `expires_at` | Claims HMAC de corta duración para deduplicación atómica y rate limiting; nunca IP ni User-Agent en claro |
...
Options principales: `bbk_schema_version`, `bbk_postview_settings`, `bbk_postview_daily_since`, `bbk_postview_migration_cursor`, `bbk_postview_migration_status`, `bbk_postview_ingestion_paused` y `bbk_postview_query_cache_generation`.
...
Actualización de fiabilidad: las pruebas históricas del buffer indicadas arriba se han
sustituido por dedupe HMAC durable, rollback ante fallo parcial, rate limit, pausa de
ingesta, límites de payload/retención y cursor de migración. `Tests/common-js.test.js`
cubre además los cinco segundos visibles y el lifecycle de pestaña. La concurrencia y
los locks reales siguen requiriendo MySQL/MariaDB.
