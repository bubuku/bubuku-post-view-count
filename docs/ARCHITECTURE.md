...
| `Api\TrendsApi` | `src/Api/TrendsApi.php` | Ruta REST `GET /bbk_postview/v1/trends`, capability `edit_posts` (`docs/ANALYTICS-PLAN.md` §4, F4) — delega en `Core\Query::trend()`. Separada de `Api\RestApi` a propósito: esa clase es el contador público anónimo con su propio modelo de seguridad; esta es de lectura y con capability, un concern distinto |
